...
$(document).on('submit', '.pass-form-normal', function () {
    var flag = false;
    var identifierError = $('.pass-form-normal .pass-generalErrorWrapper .pass-generalError-error');
    var identifier = $('.pass-form-normal .pass-text-input-userName').val();
    var credential = $('.pass-form-normal .pass-text-input-password').val();
    if (identifier === null || identifier === '') {
        console.log("username");
        identifierError.text('');
        identifierError.append("请您输入手机/邮箱/用户名");
        flag = false;
    } else if (credential === null || credential === '') {
        console.log("password");
        identifierError.text('');
        identifierError.append("请您输入密码");
        flag = false;
    } else {
...
function login(identifier, credential, flag, check) {
    var j_login = $('#j-login');
    var sms = $('#sms');
    var text_VerifyCode = $('.pass-form-normal .pass-form-item-verifyCode .pass-text-input-verifyCode');
    var form_VerifyCode = $('.pass-form-normal .pass-form-item-verifyCode');
    if (nowCity !== beforeCity) {
        ...
        $.ajax({
            ...
            error: function (data) {
                ...
                num++;
                if (num >= 3) {
                    form_VerifyCode.css('display', 'block');
                    form_VerifyCode.css('visibility', 'visible');
                    text_VerifyCode.css('color', '#F69');
                    text_VerifyCode.css('border-color', '#F69');
                }
            }
        })
    }
...
$(document).on('blur', '.pass-text-input-smsPhone', function () {
    var smsGeneralError = $('#smsForm .pass-generalErrorWrapper .pass-generalError');
    var identifier = $('.pass-text-input-smsPhone').val();
    if (identifier.length > 0) {
        if (!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(identifier))) {
            smsGeneralError.text('');
            smsGeneralError.append("手机号码格式不正确");
        }
    }
});
    ...
    if (phone.length > 0) {
        if (!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(phone))) {
            ...
            $.ajax({
                ...
                error: function (data) {
                    smsGeneralError.text('');
                    smsGeneralError.append("该手机号还未注册,请先注册");
                }
            })
        }
    } else {
        smsGeneralError.text('');
        smsGeneralError.append("请您输入手机号");
    }
});

function sendCode() {
    var buttonVerifyCodeSend = $(".pass-button-verifyCodeSend");
    if (time === 0) {//重新获取验证码
        buttonVerifyCodeSend.attr("disabled", false);
        buttonVerifyCodeSend.val("获取短信验证码");
        time = 60;
        return false;//清除定时器
    } else {
        buttonVerifyCodeSend.attr("disabled", true);
        buttonVerifyCodeSend.val("重新发送 " + time + "s");
        time--;
    }
    //设置一个定时器
    setTimeout(function () {
...
$(document).on('submit', '#smsForm', function () {
    ...
    if (identifier === null || identifier === "") {
        ...
        smsGeneralError.text('');
        smsGeneralError.append("请先输入验证码");
    } else {
        $.ajax({
            url: "/smsLogin",
            type: "post",
